...
The `leo account sign` command enables developers and users to create cryptographic signatures using an Aleo private key. These signatures can be verified within leo using the [`signature::verify`](../language/operators/cryptographic_operators.mdx#signatureverify) function or with the `leo account verify` command.
