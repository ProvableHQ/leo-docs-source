...
The keyword `program` indicates the name of the [program](./../language/project_layout/programs.mdx#program-id) inside the Leo file. In this case, it is `hello.aleo`. As mentioned before, this program name must match the one in the `program.json` manifest file.
...
For now, we'll leave it as is, which will prevent upgrades from occurring. For more details on how program upgradability works, and different patterns for upgrading your programs, check out [Upgrading Programs](../guides/program_upgradability.mdx).
...
We have dedicated guides for both [Deploying](../guides/deploying.mdx) and [Executing](../guides/executing.mdx), so please check those out for more information!
...
To learn more about the Leo language and its syntax, start [here](../language/overview.mdx).

To learn more about how to use the Leo CLI, start [here](../cli/overview.mdx).
