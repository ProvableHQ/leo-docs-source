...
An entry `fn` is the program's **public interface**. Declared inside `program {}`. It can call helper `fn` and include `final { }` blocks for onchain state updates.
...
### Entry `fn` with `final { }` (onchain state)

An entry `fn` that also modifies **public onchain state** returns `Final` and includes a `final { }` block.
