...
Specifies the private key to use for the devnode. Overrides any `$PRIVATE_KEY` environment variable set manually or in a `.env` file.

#### `--verbosity <VERBOSITY>`

#### `-v <VERBOSITY>`

Sets the log verbosity level. Valid values are `0` (silent), `1` (minimal), and `2` (verbose). Defaults to `2`.

#### `--socket-addr <SOCKET_ADDR>`

#### `-a <SOCKET_ADDR>`

Sets the address and port for the REST API. Defaults to `127.0.0.1:3030`.

#### `--genesis-path <GENESIS_PATH>`

#### `-g <GENESIS_PATH>`

Path to a custom genesis block file. If not specified, the built-in genesis block is used.

#### `--manual-block-creation`

#### `-m`

Disables automatic block creation after broadcast. Blocks will only advance when explicitly requested via `leo devnode advance`.

#### `--ledger-path <LEDGER_PATH>`

#### `-l <LEDGER_PATH>`

Directory for persistent ledger storage. If `-l` is given without a path, defaults to `./devnode/`. If not specified, the ledger is kept in memory and discarded when the devnode exits.

#### `--clean`

#### `-c`

Clears the ledger directory before starting. Requires `--ledger-path`.
...
The number of blocks to create. Defaults to `1`.

### Flags

#### `--socket-addr <SOCKET_ADDR>`

The address of the running devnode. Defaults to `127.0.0.1:3030`.
